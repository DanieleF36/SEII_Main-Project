 ...
 * @returns thesis
 **/
exports.advancedResearchThesis = async function (
  page,
  order,
  title,
  supervisor,
  ...
  if (supervisor != null) {
    ...
    if (idSupervisors != null && idSupervisors.length > 0) ok = true;
  }
  ...
  if (coSupervisor != null) {
    for (let i = 0; i < coSupervisor.length; i++) {
      ...
      if (idsCo > 0)
        ...
        idCoSupervisorsThesis.push(
          await coSupervisorThesisRepository.findThesisByCoSupervisorId(idsCo)
        );
  ...
  //find all thesis
  let res = await thesisRepository.advancedResearch(
    nItem * (page - 1),
    nItem * page,
    order,
    false,
  ...
  //find number of page
  let npage = await thesisRepository.numberOfPage(
    false,
    title,
    idSupervisors,
    idCoSupervisorsThesis,
